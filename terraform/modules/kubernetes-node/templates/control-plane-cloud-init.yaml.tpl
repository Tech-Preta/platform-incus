...
runcmd:
  ...
  %{~ else ~}
  - [ sh, -c, 'until (curl -ksS -m 5 -o /dev/null https://${common_config.bootstrap_server}:6443); do echo Wait for master node && sleep 10; done;']
  %{~ endif ~}
  ...
  - KUBECONFIG=/etc/rancher/rke2/rke2-remote.yaml /var/lib/rancher/rke2/bin/kubectl config set-cluster default --server https://${common_config.bootstrap_server}:6443
